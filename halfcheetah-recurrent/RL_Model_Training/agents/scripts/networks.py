...
## Modified by Sandeep Singh Sandha, UCLA, by adding recurrent to value network as well
...
def recurrent_gaussian(
  ...
  cell = tf.contrib.rnn.GRUBlockCell(config.policy_layers[-1]/2)
  cell2 = tf.contrib.rnn.GRUBlockCell(config.policy_layers[-1]/2)
  ...
  if state!=None:
      state1, state2 = tf.split(state, 2, axis=1)
  ...
  with tf.variable_scope('policy'):
    ...
    x, state1 = tf.nn.dynamic_rnn(cell, x, length, state1, tf.float32)
  ...
  with tf.variable_scope('value'):
    x = flat_observations
    for size in config.value_layers[:-1]:
      x = tf.contrib.layers.fully_connected(x, size, tf.nn.relu)

    x, state2 = tf.nn.dynamic_rnn(cell2, x, length, state2, tf.float32)
  ...
  value = tf.check_numerics(value, 'value')
  policy = CustomKLDiagNormal(mean, std)
  state = tf.concat([state1, state2], 1)
  #print('State after concat is:', state)
